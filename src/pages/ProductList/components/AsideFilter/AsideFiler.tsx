...
function AsideFilter({ queryConfig, categories }: Props) {
  ...
  const handleRemoveAll = () => {
    navigate({
      pathname: '/',
      search: createSearchParams(omit(queryConfig, ['price_min', 'price_max', 'rating'])).toString()
    })
  }
    ...
    <div className='py-4'>
      <Link
        ...
        className={classNames('flex items-center font-bold', {
          'font-semibold text-orange': !categoryId
        })}
      >
      ...
      <ul>
        {categories.map((categoryItem) => {
          const isActive = categoryId == categoryItem.id.toString()
          return (
      ...
      <div className='my-4 h-[1px] bg-gray-300'></div>
      <div className='text-base'>{t(' カスタマーレビュー')}</div>
